...
{$ENDIF}
    ...
    procedure LockActor(X, Y: Integer; CurrentMap: string; nRecogId: Integer = 0);//关键字锁人
    procedure GetLockActor(dir: Integer; deco: Integer; X, Y: Integer; str: string); //关键字锁人
  end;
...
{$IFEND}
  ...
  case Key of //关键字锁人  整个句子
{$IF Var_Interface = Var_Mir2}
    VK_OEM_3: begin
        if ((GetTickCount - g_dwLatestSpellTick) > g_dwMagicDelayTime) then begin
          LockActor(g_MySelf.m_nCurrX, g_MySelf.m_nCurrY, g_LegendMap.Title);
          ActionKey := byte(Key);
        end;
      end;
    VK_ESCAPE: begin
        if ((GetTickCount - g_dwLatestSpellTick) > g_dwMagicDelayTime) then begin
          g_TempLockTarget := nil;
//          g_MagicTarget := nil;
//          g_MagicLockTarget := nil;
          ActionKey := byte(Key);
        end;
...
{$IF Var_Interface =  Var_Default} //关键字锁人
      VK_ESCAPE: begin
  ...
  if (GetSpellPoint(pcm) <= g_MySelf.m_Abil.MP) or (pcm.Def.Magic.btEffectType = 0) then begin
    ...
    if pcm.Def.Magic.btEffectType = 0 then begin
      ...
      if (pcm.Def.Magic.wMagicId = 100) and (g_MySelf.m_btJob <> 0) then begin
        ...
        if g_TempLockTarget <> nil then  begin //关键字锁人
          g_TargetCret := g_TempLockTarget;
          SendSpellMsg(CM_SPELL, g_MySelf.m_btDir {x}, 0, pcm.Def.Magic.wMagicId, 0);
        end else SendSpellMsg(CM_SPELL, g_MySelf.m_btDir {x}, 0, pcm.Def.Magic.wMagicId, 0);
      end;
      g_CboMagicID := -1;
    end
    else begin
  ...
  try
    ...
    if ssRight in Shift then begin //鼠标右键
      ...
      if (abs(g_MySelf.m_nCurrX - g_nMouseCurrX) <= 1) and (abs(g_MySelf.m_nCurrY - g_nMouseCurrY) <= 1) then begin
       ...
       if (not g_LockTarget) then begin //关键字锁人
        tdir := GetNextDirection(g_MySelf.m_nCurrX, g_MySelf.m_nCurrY, g_nMouseCurrX, g_nMouseCurrY);
        ...
        Exit;
       end else if target = nil then begin  //关键字锁人
        tdir := GetNextDirection(g_MySelf.m_nCurrX, g_MySelf.m_nCurrY, g_nMouseCurrX, g_nMouseCurrY);
        nX := g_MySelf.m_nCurrX;
        nY := g_MySelf.m_nCurrY;
        if g_MySelf.m_btHorse <> 0 then begin
        ...
        Exit;
       end else begin //关键字锁人
         LockActor(g_MySelf.m_nCurrX, g_MySelf.m_nCurrY, g_LegendMap.Title, target.m_nRecogId);
       end;
      end;
    end;
    //摆摊不允许操作左键
    if (ssLeft in Shift) {Button = mbLeft} and (not g_MySelf.m_boShop) then begin//鼠标左键
      ...
      PlayScene.CXYfromMouseXY(X, Y, g_nMouseCurrX, g_nMouseCurrY);
      if not g_LockTarget then  //关键字锁人
        g_TargetCret := nil
      else begin
        g_TargetCret := nil;
        g_MagicLockTarget := nil;
          ...
          if target <> nil then begin
            ...
            if (not target.m_boDeath) and (not target.m_boShop) and ((g_MySelf.m_btHorse = 0) or (g_UseItems[U_HOUSE].s.StdMode2 = 33) or (target.m_btRaceServer = RC_BOX)) then begin
             if not g_LockTarget then //关键字锁人
              ...
              g_TargetCret := target;
              if target.m_btRace <> 50 then begin
                LockActor(g_MySelf.m_nCurrX, g_MySelf.m_nCurrY, g_LegendMap.Title, target.m_nRecogId);
              end;
             end;
...
{$IFEND}
    ...
    SM_LOCKACTOR: begin //关键字锁人
        GetLockActor(Msg.Recog, Msg.Series, Msg.Param, Msg.tag, DecodeString(body));
      end;
  else begin
      if g_MySelf = nil then
...
//关键字锁人
procedure TfrmMain.LockActor(X, Y: Integer; CurrentMap: string; nRecogId: Integer);
var
  Msg: TDefaultMessage;
begin
  Msg := MakeDefaultMsg(CM_LOCKACTOR, nRecogId, 0, X, Y);
  SendSocket(EncodeMessage(Msg) + EncodeString(CurrentMap));     
end;
//关键字锁人
procedure TfrmMain.GetLockActor(dir: Integer; deco: Integer; X, Y: Integer; str: string);
var
  target: TActor;
  STR1, sMAP, sGuild, sGroup: string;
  sMAPs, sGuilds, sGroups: string;
begin
  str := GetValidStr3(str, STR1, ['/']);
  str := GetValidStr3(str, sMAPs, ['/']);
  str := GetValidStr3(str, sGuilds, ['/']);
  str := GetValidStr3(str, sGroups, ['/']);
  target := PlayScene.FindActor(dir);
  if (target <> nil) and (target <> g_MySelf) then begin
    g_TempLockTarget := target;
    DecodeBuffer(STR1, @g_TempLockTarget.m_Abil, sizeof(TAbility));
    sMAP := DecodeString(sMAPs);
    sGuild := DecodeString(sGuilds);
    sGroup := DecodeString(sGroups);
    g_TempLockTarget.m_nDeco := deco;//头像
    g_MagicTarget := target;
    g_MagicLockTarget := target;
    if (g_TempLockTarget.m_btRace = 0) then begin
      case FrmDlg3.DComboBoxSay.ItemIndex of
        1: begin
          sendsay('_附近_目标: ' + g_TempLockTarget.m_UserName + '<' + sMAP + '(X:' + IntToStr(X) + ',' + 'Y:' + IntToStr(Y) + ')>');
        end;
        2: begin
          if sGroup <> '0' then
          sendsay('!!_组队_目标: ' + g_TempLockTarget.m_UserName + '<' + sMAP + '(X:' + IntToStr(X) + ',' + 'Y:' + IntToStr(Y) + ')>');
        end;
        3: begin
          if sGuild <> '0' then
          sendsay('!~_行会_目标: ' + g_TempLockTarget.m_UserName + '<' + sMAP + '(X:' + IntToStr(X) + ',' + 'Y:' + IntToStr(Y) + ')>');
        end;
      end;
