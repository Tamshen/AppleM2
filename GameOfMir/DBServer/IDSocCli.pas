...
uses
  ...
  Controls, Forms, Dialogs, StdCtrls, ExtCtrls, JSocket, Grobal2, DBShare,
    IniFiles, Common;
type
...
begin
  ...
  while (Pos(')', sScoketText) > 0) do begin
    ...
    case nIdent of
      ...
      SS_CREATENEWCHR: FrmUserSoc.AddNewChr(sBody);
    end;
  end;
...
var
  ...
  sData: string;
begin
  if not IDSocket.Socket.Connected then begin
    IDSendStr := '';
    exit;
  end;
  ...
  {IDSendStr := IDSendStr + sSENDMSG;
  if IDSendStr = '' then exit;
  nLen := Length(IDSendStr);
  nSendLen := 0;
  if IDSocket.Socket.Connected then
    nSendLen := IDSocket.Socket.SendText(sSENDMSG);
  if (nSendLen = nLen) or (nSendLen = 0) then begin
    IDSendStr := '';
  end else
  if (nSendLen < nLen) then begin
    IDSendStr := Copy(IDSendStr, nSendLen + 1, nLen - nSendLen);
  end;   }
end;
...
begin
  SendSocket('(' + IntToStr(SS_SERVERINFO) + '/' + sServerName +
