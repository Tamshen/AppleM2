      ...
      try
        if (GetTickCount - dwRefConsoleMsgTick) >= 1000 then begin
          ...
          if not boShowBite then begin
            LabelReviceMsgSize.Caption := '接收: ' + IntToStr(nReviceMsgSize div 1024);
            LabelBufferOfM2Size.Caption := '服务器通讯: ' + IntToStr(nBufferOfM2Size div 1024);
            LabelProcessMsgSize.Caption := '编码: ' + IntToStr(nProcessMsgSize div 1024);
            LabelLogonMsgSize.Caption := '登录: ' + IntToStr(nHumLogonMsgSize div 1024);
            LabelPlayMsgSize.Caption := '数据: ' + IntToStr(nHumPlayMsgSize div 1024);
          end
          else begin
            LabelReviceMsgSize.Caption := '接收: ' + IntToStr(nReviceMsgSize);
            LabelBufferOfM2Size.Caption := '服务器通讯: ' + IntToStr(nBufferOfM2Size);
            LabelProcessMsgSize.Caption := '编码: ' + IntToStr(nProcessMsgSize);
            LabelLogonMsgSize.Caption := '登录: ' + IntToStr(nHumLogonMsgSize);
            LabelPlayMsgSize.Caption := '数据: ' + IntToStr(nHumPlayMsgSize);
          end;
  ...
  try
    ...
    if (UserData.nSocketIdx >= 0) and (UserData.nSocketIdx < RUNATEMAXSESSION) then begin
      if (UserData.nSocketHandle = SessionArray[UserData.nSocketIdx].nSckHandle) and
              ...
              if (nDataLen >= DEFBLOCKSIZE) then begin
                if SessionArray[UserData.nSocketIdx].boStartLogon then begin
                  ...
                  if (GetCertification(sData, sAccount, sChrName, nSessionID, nClientVersion, boFlag)) and
                    ...
                    AddMainLogMsg('[全局验证失败]：' + SessionArray[UserData.nSocketIdx].sRemoteAddr, 2);
                    SessionArray[UserData.nSocketIdx].sSocData := '';
                  ...
                  if (DefMsg.Ident > RG_MAXMSGINDEX) or (DefMsg.Ident < RG_MINMSGINDEX) then begin
                    AddMainLogMsg('非法代号: ' + IntToStr(DefMsg.Ident), 3);
                    SessionArray[UserData.nSocketIdx].sSocData := '';
                    AddAttackInfo(SessionArray[UserData.nSocketIdx].sRemoteAddr, SessionArray[UserData.nSocketIdx].Socket);
                    Exit;
                  end;
        ...
        AddMainLogMsg('数据包攻击: ' + SessionArray[UserData.nSocketIdx].sRemoteAddr + ' ' +
      ...
      if nSendLen > MAXSOCKETBUFFLEN then begin
        ...
        if UserSession.Socket.SendText(sData) <> -1 then begin
          ...
          if UserSession.dwSuspendedCount >= 100 then begin
            ...
            AddMainLogMsg('断开连接: 传输失败 ' + UserSession.sRemoteAddr, 3);
          end;
          break;
        end;
      end
...
begin
  ...
  if boGateReady and boCenterReady then begin
    ...
    if IsBlockIP(sRemoteAddress) then begin
      Inc(nCCHeadOffCount);
      AddMainLogMsg('过滤连接: ' + sRemoteAddress, 3);
      Socket.Close;
      Exit;
    end;
    ...
    if IsConnLimited(sRemoteAddress, Socket.SocketHandle) then begin
      ...
      AddMainLogMsg('端口攻击: ' + sRemoteAddress, 3);
      Exit;
    end;
    ...
    if Socket.nIndex > -1 then begin
      ...
      Socket.nIndex := -1;
      Socket.Close;
      AddMainLogMsg('连接满员: ' + sRemoteAddress, 3);
    end;
  end
  else begin
    Socket.nIndex := -1;
    Socket.Close;
    AddMainLogMsg('禁止连接: ' + sRemoteAddress, 3);
  end;
end;
      ...
      if nReviceLen > nNomClientPacketSize then begin
        ...
        if (nMsgCount > nMaxClientMsgCount){ or (nReviceLen > nMaxClientPacketSize)} then begin
          if bokickOverPacketSize then begin
            AddMainLogMsg('踢除连接: IP(' + sRemoteAddress + '),信息数量(' + IntToStr(nMsgCount) + ')', 1);
...
procedure TFrmMain.ServerSocketListen(Sender: TObject;
  Socket: TCustomWinSocket);
begin
  AddMainLogMsg('端口绑定(' + Socket.LocalAddress + ':' + IntToStr(Socket.LocalPort) + ')...', 0);
end;
    ...
    for i := 0 to RUNATEMAXSESSION - 1 do begin
      ...
      if UserSession.Socket <> nil then begin
        if UserSession.boStartLogon and ((GetTickCount - UserSession.dwSendClientCheckTick) > dwClientTimeOutTime) then begin
          ...
          UserSession.Socket := nil;
          UserSession.nSckHandle := -1;  }
          AddMainLogMsg('空连接:  ' + UserSession.sRemoteAddr, 3);
          ...
          UserSession.Socket := nil;
          UserSession.nSckHandle := -1;
          AddMainLogMsg('断开连接: 无传输超时 ' + UserSession.sRemoteAddr, 3);
        end;
...
begin
  if not boGateReady then
    AddMainLogMsg('连接到(' + Socket.RemoteAddress + ':' + IntToStr(Socket.RemotePort) + ')...', 0);
  boGateReady := True;
  dwCheckServerTick := GetTickCount();
  RestSessionArray();
  boServerReady := True;
...
begin
  if boGateReady then
    AddMainLogMsg('已断开(' + Socket.RemoteAddress + ':' + IntToStr(Socket.RemotePort) + ')...', 0);
  for i := 0 to RUNATEMAXSESSION - 1 do begin
    UserSession := @SessionArray[i];
    if UserSession.Socket <> nil then begin
      UserSession.Socket.Close;
...
begin
  if Application.MessageBox('是否确认重新加载配置信息？', '确认信息', MB_OKCANCEL + MB_ICONQUESTION) <> IDOK
    then
    Exit;
  LoadConfig();
  AddMainLogMsg('重新加载配置完成...', 0);
end;
...
begin
  if boOpen then begin
    ...
    AddMainLogMsg('CC防御已关闭，共拦截 '+ IntToStr(nCCHeadOffCount) +' 次攻击...', 0);
  end;
end;
