...
interface
uses
  Windows, Classes, SysUtils, IniFiles, Grobal2, ObjBase, ObjMon2, Guild, ObjPlay, Envir;
const
  MAXCASTLEARCHER = 12;
  MAXCALSTEGUARD = 4;
type
...
//    m_AttackGuildList: TList; //0x50
  ...
  public
    ...
    procedure StartWallconquestWarOnAllGuild;
  ...
  if g_MapManager.GetMapOfServerIndex(m_sMapName) = nServerIndex then begin
    ...
    if m_MapCastle <> nil then begin
      ...
      if m_MainDoor.BaseObject <> nil then begin
        ...
        if m_MainDoor.nStatus then TCastleDoor(m_MainDoor.BaseObject).Open
        else TCastleDoor(m_MainDoor.BaseObject).Close;
...
{$IF SoftVersion <> VERDEMO}
    ...
    if m_boUnderWar then begin
      ...
      if not m_boShowOverMsg then begin //00490181
        if (GetTickCount - m_dwStartCastleWarTick) > (g_Config.dwCastleWarTime - g_Config.dwShowCastleWarEndMsgTime) then begin
      ...
      if ((GetTickCount - m_dwStartCastleWarTick) > g_Config.dwGetCastleTime) and (GetTickCount > m_dwGetCastleTick) then
      begin
        m_dwGetCastleTick := GetTickCount + 1000;
        CheckOccupancyGuild();
      end;
      if (GetTickCount - m_dwStartCastleWarTick) > g_Config.dwCastleWarTime then begin
        StopWallconquestWar();
      end;
    end
    else begin
...
begin
  ...
  m_MasterGuild := Guild;
  m_sOwnGuild := Guild.m_sGuildName;
  m_ChangeDate := Now();
  SaveConfigFile();
...
procedure TUserCastle.StartWallconquestWarEx;
var
  i: Integer;
  Year, Month, Day, Hour, Min, Sec, MSec: Word;
  wYear, wMonth, wDay: Word;
...
procedure TUserCastle.StartWallconquestWarOnAllGuild;
var
  i: Integer;
  Year, Month, Day, Hour, Min, Sec, MSec: Word;
  wYear, wMonth, wDay: Word;
...
begin
  ...
  for I := 0 to g_GuildManager.m_GuildList.Count - 1 do begin
    Guild := TGuild(g_GuildManager.m_GuildList.Items[I]);
    Guild.m_AttackCastle := Self;
  end;
  if m_boUnderWar then begin
    if m_MasterGuild <> nil then
      m_MasterGuild.m_AttackCastle := Self;
...
begin
  Result := False;
  if (not m_boUnderWar) or (Guild = nil) then Exit; //如果未开始攻城，则无效
...
procedure TUserCastle.CheckOccupancyGuild;
var
  i: Integer;
  List14: TList;
  PlayObject: TPlayObject;
...
begin
  ...
  boOccupancy := False;
  Guild := nil;
  if List14.Count > 0 then begin
    Guild := TGuild(TPlayObject(List14.Items[0]).m_MyGuild);
    if (Guild <> nil) and (Guild <> m_MasterGuild) and (Guild.m_AttackCastle = Self) then begin
      boOccupancy := True;
      for i := 1 to List14.Count - 1 do begin
        PlayObject := TPlayObject(List14.Items[i]);
        if (PlayObject.m_MyGuild = nil) or (PlayObject.m_MyGuild <> Guild) then begin
          boOccupancy := False;
          break;
        end;
      end;
  ...
  List14.Free;
  if boOccupancy and (Guild <> nil) then GetCastle(Guild);
end;
...
begin
  ...
  Try
    ...
    AttackerInfo.sGuildName := Guild.m_sGuildName;
    AttackerInfo.Guild := Guild;
    m_AttackWarList.Add(AttackerInfo);
    nCode := 2;
    if m_MasterGuild <> nil then
    ...
    SaveAttackSabukWall();
    nCode := 4;
    if (m_MasterGuild <> nil) and (m_MasterGuild.IsAllyGuild(Guild)) then begin
      nCode := 5;
    ...
    On E:Exception do begin
      MainOutMessage('[Exception] TUserCastle.AddAttackerInfo ' + IntToStr(nCode));
