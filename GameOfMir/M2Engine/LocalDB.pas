...
var
  sFileName, sLineText, sScript, sMapName, sX, sY, sName, sFlag, sAppr, sHintName, sDir, sAppr2, 
    sIsCalste, sCanMove, sMoveTime, sAutoChangeColor, sAutoChangeColorTime, sDeco: string;
  ...
  if MyFileExists(sFileName) then begin
    ...
    for i := 0 to tMerchantList.Count - 1 do begin
      ...
      if (sLineText <> '') and (sLineText[1] <> ';') then begin
        ...
        sLineText := GetValidStr3(sLineText, sDeco, [' ', #9]); //关键字锁人
          ...
          if sMapName[1] = '$' then begin
            ...
            if nIdx <> -1 then begin
              ...
              for k := 0 to FBList.Count - 1 do begin
                ...
                tMerchantNPC.m_bDeco := StrToIntDef(sDeco, 0); //关键字锁人
...
begin
  ...
  try
    ...
    for i := 0 to Query.RecordCount - 1 do begin
      ...
      Monster.wDeco := _MAX(0, Query.FieldByName('Deco').AsInteger);//关键字锁人
