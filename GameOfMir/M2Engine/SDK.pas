...
type
  ...
  TInitPlug = procedure(LoadMode: Boolean); stdcall;

  TPlugLoad = function(AppHandle: HWnd; MsgProc: TMsgProc; FindProc: TFindProc;
    SetProc: TSetProc; FindOBj: TFindObj; LoadMode: Boolean): PChar; stdcall;
  ...
  TSetFriend = procedure(AddBaseObject, NameBaseObject: TObject; nCheck: Integer); stdcall;
  //TSetEMailInfo = procedure(boLoad, boLoadFail, boRun, boGhost: Boolean; nDBIndex: Integer); stdcall;
  TDeCryptString = procedure(Src, Dest: PChar; nSrc: Integer; var nDest: Integer); stdcall;
  TUnModule = function(ModuleHookType: Integer; OldPointer, NewPointer: Pointer): Boolean; stdcall;
      ...
      if FileHandle > 0 then begin
        if FileRead(FileHandle, nItemCount, SizeOf(TItemCount)) = SizeOf(TItemCount) then begin
          ...
          for i := 0 to nItemCount - 1 do begin
            New(BigStorage);
            SafeFillChar(BigStorage.UseItems, SizeOf(TUserItem), #0);
  ...
  try
    ...
    for i := 0 to m_BigStorageList.Count - 1 do begin
      ...
      if (List <> nil) and (List.Count > 0) then begin
        ...
        if CompareText(Storage.sCharName, sChrName) = 0 then begin
          New(AddStorage);
          SafeFillChar(AddStorage.UseItems, SizeOf(TUserItem), #0);
...
//      nChangeCount := m_nChangeCount;
      ...
      if FileHandle > 0 then begin
        SafeFillChar(nItemCount, SizeOf(TItemCount), #0);
      ...
      if FileHandle > 0 then begin
        if FileRead(FileHandle, Header420, SizeOf(TSellOffHeader)) = SizeOf(TSellOffHeader) then begin
          ...
          for i := 0 to Header420.nItemCount - 1 do begin
            New(SellOffInfo);
            SafeFillChar(SellOffInfo.UseItems, SizeOf(TUserItem), #0);
...
//      nChangeCount := m_nChangeCount;
      ...
      if FileHandle > 0 then begin
        SafeFillChar(Header420, SizeOf(TSellOffHeader), #0);
        for i := m_SellOffGoodList.Count - 1 downto 0 do begin
          List := TList(m_SellOffGoodList.Items[i]);
          if List <> nil then begin
            if List.Count <= 0 then begin
