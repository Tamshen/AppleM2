...
uses
  ...
  Menus, IniFiles, Common, GateShare, AppEvnts, Grobal2;
type
  TUserSession = record
...
begin
  DelMapHandle(gt_SelGate, Handle);
  TempLogList.Free;
end;

procedure TFrmMain.F1Click(Sender: TObject);
begin
  F1.Checked := not F1.Checked;
end;
...
begin
  ...
  UserSession.sReadString := '';
  if Length(sMsg) > 100 then begin sMsg := '';
    MainOutMessage('超长数据攻击: ' + UserSession.sRemoteIPaddr + ' ' + IntToStr(Length(sMsg)), 3);
    AttackDispose(UserSession.sRemoteIPaddr, UserSession.Socket);
    Exit;
  end;
  while (sMsg <> '') do begin
      ...
      if (length(sCode) >= DEFBLOCKSIZE + 1) then begin
        if boCheckClientMsg then
          ...
          if (Pos(MG_CodeHead, sCode) > 0) or
            ...
            MainOutMessage('#号攻击: ' + UserSession.sRemoteIPaddr, 3);
            AttackDispose(UserSession.sRemoteIPaddr, UserSession.Socket);
            sMsg := '';
            break;
          end
...
begin
  if g_boTeledata then sConfigFileName := TeledataConfigFileName
  else sConfigFileName := ConfigFileName;
...
begin
  ...
  nCCConnCount := 0;
  boOpenCCProtect := False;
  AddMapHandle(gt_SelGate, Handle);
...
begin
  if boOpen then begin
    ...
    MainOutMessage('侦测到CC攻击，已开启防御...', 0);
  end else begin
    ...
    MainOutMessage('CC防御已关闭，共拦截 '+ IntToStr(nCCHeadOffCount) +' 次攻击...', 0);
  end;
end;
