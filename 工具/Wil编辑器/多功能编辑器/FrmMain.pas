...
begin
  OpenDialog.FileName := '';
  OpenDialog.Filter := '传奇二图像存储文件(565)|*_16.wil';
...
begin
  OpenDialog.FileName := '';
  OpenDialog.Filter := '传奇二标准图像存储文件|*.wil';
...
begin
  OpenDialog.FileName := '';
  OpenDialog.Filter := '传奇二加密图像存储文件|*.wis';
...
begin
  OpenDialog.FileName := '';
  OpenDialog.Filter := '传奇二压缩图像存储文件|*.wzl';
...
begin
  OpenDialog.FileName := '';
  OpenDialog.Filter := '传奇三标准图像存储文件|*.wil';
...
begin
  OpenDialog.FileName := '';
  OpenDialog.Filter := '传奇三压缩图像存储文件|*.miz';
...
begin
  OpenDialog.FileName := '';
  OpenDialog.Filter := '传奇世界图像数据存储文件|*.sgl';
...
begin
  OpenDialog.FileName := '';
  OpenDialog.Filter := '武易图像数据存储文件|*.521g';
...
begin
  OpenDialog.FileName := '';
  OpenDialog.Filter := '自定义存储文件|*' + MYFILEEXT;
...
begin
  if g_WMImages <> nil then begin
    ...
    if g_WMImages is TWMM2DefImages then begin
      if TWMM2DefImages(g_WMImages).bo16bit then
        StatusBar.Panels[1].Text := '格式: MIR2 标准数据格式(16位)'
      else
      if TWMM2DefImages(g_WMImages).NewFmt then
        StatusBar.Panels[1].Text := '格式: MIR2 标准数据格式(新)'
      else
        StatusBar.Panels[1].Text := '格式: MIR2 标准数据格式(旧)';
    end
    else if g_WMImages is TWMM2DefBit16Images then
      StatusBar.Panels[1].Text := '格式: MIR2 特定数据格式(16位)'
    else if g_WMImages is TWMM2ZipImages then
      StatusBar.Panels[1].Text := '格式: MIR2 标准压缩格式'
    else if g_WMImages is TWMM2WisImages then
      StatusBar.Panels[1].Text := '格式: MIR2 特定加密模式'
    else if g_WMImages is TWMWoolDefImages then
      StatusBar.Panels[1].Text := '格式: 传奇世界'
    else if g_WMImages is TWMM3DefImages then begin
      if TWMM3DefImages(g_WMImages).NewFmt then
        StatusBar.Panels[1].Text := '格式: MIR3 标准数据格式(新)'
      else
        StatusBar.Panels[1].Text := '格式: MIR3 标准数据格式(旧)';
    end
    else if g_WMImages is TWMM3ZipImages then
      StatusBar.Panels[1].Text := '格式: MIR3 标准压缩格式'
    else if g_WMImages is TWMMyImageImages then begin
      StatusBar.Panels[1].Text := '格式: 自定义图像文件模式'
    end;
  end
  else begin
  ...
  StatusBar.Panels[7].Text := Format('测试: (%d,%d)', [FShowX, FShowY]);
end;
