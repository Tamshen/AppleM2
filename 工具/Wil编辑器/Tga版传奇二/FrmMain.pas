...
uses
  ...
  Dialogs, ExtDlgs, ExtCtrls, WinSkinData, Grids, StdCtrls, WIL, GraphicEx;
...
var
  ...
  HotStartKeyID: Integer;
  HotStopKeyID: Integer;
  FullCanvas: TCanvas;
  FaceDib: TDIB;
  ...
  if WMImages.boInitialize then begin
    if OpenPictureDialog.Execute(Handle) then begin
      if OpenPictureDialog.FileName <> '' then begin
        ...
        try
          TargaGraphic.LoadFromFile(OpenPictureDialog.FileName);
...
begin
  if msg.HotKey = HotStartKeyID then begin
    FaceDib.Canvas.CopyRect(Rect(0, 0, 20, 20), FullCanvas, Rect(395, 134, 415, 154));
    FormMain.WMImages.AddImages(0, 0, FaceDib);
    FormMain.WMImages.SaveIndex;
    FormMain.DrawGrid.RowCount := FormMain.WMImages.ImageCount div 6 + 1;
    FormMain.DrawGrid.Row := FormMain.DrawGrid.RowCount - 1;
    DrawGrid.Col := DrawGrid.ColCount - 1;
    FormMain.DrawGrid.Repaint;
  end
  else if msg.HotKey = HotStopKeyID then begin
...
var
  Res: TResourceStream;
begin
  HotStartKeyID := GlobalAddAtom('StartKey');
  HotStopKeyID := GlobalAddAtom('StopKey');
  RegisterHotKey(Handle, HotStartKeyID, 0, VK_F8);
  RegisterHotKey(Handle, HotStopKeyID, 0, VK_F9);
  FullCanvas := TCanvas.Create;
  FullCanvas.Handle := GetDC(0);
  FaceDib := TDIB.Create;
  FaceDib.BitCount := 32;
  FaceDib.Width := 20;
  FaceDib.Height := 20;
...
begin
  UnregisterHotKey(Handle, HotStartKeyID);
  UnregisterHotKey(Handle, HotStopKeyID);
  DeleteAtom(HotStartKeyID);
  DeleteAtom(HotStopKeyID);
  FullCanvas.Free;
  FaceDib.Free;
  if WMImages.boInitialize then
    WMImages.Finalize;
