...
begin
  if OpenDialog.Execute(Handle) then begin
    if OpenDialog.FileName <> '' then begin
      ...
      CKSetTransparentColor.Enabled := (WMImages.Version = 2) and (WMImages.BitCount = 16) and (WMImages.Version <> 3);
      EdTransparentColor.Enabled := (WMImages.Version = 2) and (WMImages.BitCount = 16) and (WMImages.Version <> 3);
...
begin
  if not WMImages.boInitialize then
    Exit;
  if WMImages.Version = 3 then begin
    if ImageInfo.nSize <> 0 then lblClass.Caption := 'MIR2  wis格式(已加密)'
    else lblClass.Caption := 'MIR2  wis格式(未加密)';
  end else
