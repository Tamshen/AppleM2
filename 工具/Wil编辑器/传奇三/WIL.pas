  ...
  if not (csDesigning in ComponentState) then begin
    ...
    if FileExists(FFileName) then begin
      FIdxFile := ExtractFilePath(FFileName) + ExtractFileNameOnly(FFileName) +
        '.WIX';
      if not LoadIndex(FIdxFile) then
        exit;
...
{$IFEND}
      ...
      if FHeader.ImageCount = m_IndexList.Count then begin
        ...
        raise Exception.Create(self.Name + ' 对应文件数量不一致');
      end;
    end;
  end;
end;
...
function TWMImages.LoadIndex(idxfile: string): Boolean;
var
...
begin
  ...
  if FileExists(idxfile) then begin
    ...
    if fhandle > 0 then begin
      ...
      if (FIdxHeader.VerFlag and $FFFF0000) = $B13A0000 then begin
        btVersion := 1;
      end
      else begin
        btVersion := 0;
...
begin
  ...
  if (imginfo.nWidth > 1024) or (imgInfo.nHeight > 768) or (imginfo.nWidth < 1)
    or (imgInfo.nHeight < 1) or boHeader then
    Exit;
  nLeng := imginfo.dwImageLength;
  ...
  lsDib.BitCount := 16;
  lsDib.BitmapInfo.bmiHeader.biCompression := BI_BITFIELDS;
  SetLength(Bits, nLeng);
  //SetLength(DBits, imginfo.nWidth * imginfo.nHeight);
  //GetMem(Pointer(DBits), imginfo.nWidth * imginfo.nHeight * 2);
  //Bits := TARRAY(Integer(PBits));
  DBits := lsDib.PBits;
  FillChar(DBits^,imginfo.nWidth * imginfo.nHeight * 2, #0 );
  nWidthStart := 0;
  nWidthEnd := 0;
  nCurrWidth := 0;
  nCntCopyWord := 0;
  ...
  nX := 0;
  try
    if m_FileStream.Read(Bits[0], nLeng * 2) = (nLeng * 2) then begin
      for nY := lsDib.Height - 2 downto 0 do begin
        nWidthEnd := nWidthEnd + Bits[nWidthStart]; //循环取字节
        Inc(nWidthStart);
        nX := nWidthStart;
        while nX < nWidthEnd do begin
          if (Bits[nx] = $C0) then begin
            Inc(nX);
            nCntCopyWord := Bits[nx];
            Inc(nX);
            nCurrWidth := nCurrWidth + nCntCopyWord;
          end
          else if (Bits[nx] = $C1) or (Bits[nx] = $C2) or (Bits[nx] = $C3) then
            begin
            INC(nx);
            nCntCopyWord := Bits[nx];
            INC(nx);
            ...
            nCurrWidth := nCurrWidth + nCntCopyWord;
            {Move(Bits[nx],DBits[(nY * lsDib.Width * 2) + (nLastWidth +
                nX) * 2] ,sizeof(WORD) * nCntCopyWord); }
            copymemory(@DBits[(nY * lsDib.Width * 2) + (nLastWidth) * 2], @Bits[nx], sizeof(WORD) * nCntCopyWord);
            nX := nX + nCntCopyWord;
            //end;
          end
...
begin
  ...
  if (index >= 0) and (index < ImageCount) and (index < m_IndexList.Count) then
    ...
    if MakeDIB(nPosition) then begin
      ...
      //SetBitmapBits(Bitmap.Handle, lsDib.Width * lsDib.Height * sizeof(WORD), lsDib.PBits);
