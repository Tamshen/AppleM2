...
type
  TfrmIPaddrFilter = class(TForm)
    ...
    EditAddNewUserTime: TSpinEdit;
    Label8: TLabel;
    Label14: TLabel;
    Label12: TLabel;
    EditChangePassword: TSpinEdit;
    Label13: TLabel;
    Label15: TLabel;
    EditLoadPassword: TSpinEdit;
    Label16: TLabel;
    Label23: TLabel;
    ...
    procedure EditAddNewUserTimeChange(Sender: TObject);
    procedure EditChangePasswordChange(Sender: TObject);
    procedure EditLoadPasswordChange(Sender: TObject);
...
var
  ...
  Blockaddr: pTBlockaddr;
begin
  if (ListBoxIpList.ItemIndex >= 0) and (ListBoxIpList.ItemIndex < ListBoxIpList.Items.Count) then
  begin
    ...
    begin
      if pTBlockaddr(IPList.Items[i]) = Blockaddr then
      begin
        IPList.Delete(i);
        Dispose(Blockaddr);
        break;
...
begin
  ...
  begin
    ...
    begin
      if pTSockaddr(TempBlockIPList.Items[I]).nIPaddr = nIPaddr then
      begin
...
begin
  if (ListBoxTempList.ItemIndex >= 0) and (ListBoxTempList.ItemIndex <
    ListBoxTempList.Items.Count) then
  begin
    sIPaddr := ListBoxTempList.Items.Strings[ListBoxTempList.ItemIndex];
    ListBoxTempList.Items.Delete(ListBoxTempList.ItemIndex);
    nIPaddr := inet_addr(PChar(sIPaddr));
...
begin
  ...
  begin
    ...
    begin
      if pTSockaddr(BlockIPList.Items[I]).nIPaddr = nIPaddr then
      begin
...
begin
  if (ListBoxBlockList.ItemIndex >= 0) and (ListBoxBlockList.ItemIndex <
    ListBoxBlockList.Items.Count) then
  begin
    sIPaddr := ListBoxBlockList.Items.Strings[ListBoxBlockList.ItemIndex];
    nIPaddr := inet_addr(PChar(sIPaddr));
    ListBoxBlockList.Items.Delete(ListBoxBlockList.ItemIndex);
...
procedure TfrmIPaddrFilter.EditAddNewUserTimeChange(Sender: TObject);
begin
  nAddNewUserTime := EditAddNewUserTime.Value;
end;

procedure TfrmIPaddrFilter.EditChangePasswordChange(Sender: TObject);
begin
  nChangePassword := editChangePassword.Value;
end;
...
procedure TfrmIPaddrFilter.EditLoadPasswordChange(Sender: TObject);
begin
  nLoadPassword := editLoadPassword.Value;
end;
...
begin
  ...
  if FrmMain.ServerSocket.Active then
    for I := 0 to FrmMain.ServerSocket.Socket.ActiveConnections - 1 do
    begin
...
begin
  ...
  Conf.WriteInteger(GateClass, 'AddNewUserTick', nAddNewUserTime);
  Conf.WriteInteger(GateClass, 'ChangePasswordTick', nChangePassword);
  Conf.WriteInteger(GateClass, 'LoadPasswordTick', nLoadPassword);
  Conf.Free;
  Close;
end;
...
begin
  ...
  EditAddNewUserTime.Enabled := CheckBoxCheckClientMsg.Checked;
  EditChangePassword.Enabled := CheckBoxCheckClientMsg.Checked;
  EditLoadPassword.Enabled := CheckBoxCheckClientMsg.Checked;
end;
