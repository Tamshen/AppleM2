...
uses
  Windows, Messages, SysUtils, StrUtils, Variants, Classes, Graphics, Controls, Forms, Share, ZLIB,
    ...
    if FileExists(g_ClientName) then begin
      if CheckMirDir(g_SelfPath, True) then begin
        ...
        if nCount > 0 then begin
          ...
          else WinExec(PChar(g_ClientName + ' ' + IntToStr(Handle) + ' 0'), SW_SHOW);
...
begin
  with TreeViewServer do begin
    Left := g_SkinInfo.ServerLIST_Rect.Left;
    Top := g_SkinInfo.ServerLIST_Rect.Top;
    Width := g_SkinInfo.ServerLIST_Rect.Width;
    Height := g_SkinInfo.ServerLIST_Rect.Height;
  end;
  WebBrowser.Width := g_SkinInfo.IE_Rect.Width;
  WebBrowser.Height := g_SkinInfo.IE_Rect.Height;
  WebBrowser.Left := g_SkinInfo.IE_Rect.Left;
  WebBrowser.Top := g_SkinInfo.IE_Rect.Top;
end;
