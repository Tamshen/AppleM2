...
type
  ...
  public
    ...
    procedure SetSize(AWidth, AHeight, ABitCount: Integer); reintroduce;
...
function pfRGBEx(OldByte:Byte): DWORD;
...
var
  DIB_NowPixelFormat:TDIBPixelFormat;
  DIB_ColorTable: TRGBQuads;
  //DIB_LowColorTable:array[0..255,0..255,0..255] of Byte;
...
begin
  with PixelFormat do
    Result := ((R shl RShift) and RBitMask) or ((G shl GShift) and GBitMask) or
      ((B shr BShift) and BBitMask) or (10 shl 24);
end;
...
var
  cR,cG,cB:Integer;
begin
  with DIB_ColorTable[OldByte] do
  begin
    cR := rgbRed;
    cG := rgbGreen;
    cB := rgbBlue;
  end;
  with DIB_NowPixelFormat do
    Result := ((cR shl RShift) and RBitMask) or ((cG shl GShift) and GBitMask) or
      ((cB shr BShift) and BBitMask) or (OldByte shl 24);
  //Result := (cR or (cG shl 8) or (cB shl 16));
  //Result:=pfRGB(DIB_NowPixelFormat, cR, cG, cB);
  {asm
    movzx   eax, OldByte
    lea   edi, Result
    mov   [edi+3],al
...
initialization
  ...
  TPicture.RegisterFileFormat('dib', 'Device Independent Bitmap', TDIB);
   with DIB_NowPixelFormat do begin
    RBitMask:=16711680;
    GBitMask:=65280;
    BBitMask:=255;
    RBitCount:=8;
