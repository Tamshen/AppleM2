...
begin
  colcount := StrToIntDef(EdCol.Text, 0);
  rowcount := StrToIntDef(EdRow.Text, 0);
...
begin
  ...
  mapprj.ColCount := StrToIntDef(EdCol.Text, 0);
  mapprj.RowCount := StrToIntDef(EdRow.Text, 0);
...
begin
  ...
  with SegGrid do begin
    for i := 0 to 2 do begin
      if (i + TopRow) >= RowCount then
        break;
      for j := 0 to 2 do begin
        if (j + LeftCol) >= ColCount then
          break;
...
begin
  if SegGrid.LeftCol > 0 then
    SegGrid.LeftCol := SegGrid.LeftCol - 1;
  BtnEditClick(self);
end;
