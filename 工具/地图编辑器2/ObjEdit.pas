...
begin
  ...
  for i := 0 to WIlcount - 1 do begin
    ...
    case i of
      ...
      1..29: CbWilIndexList.Items.Add(format('Objects%d.wil', [i + 1]));
      30..39: CbWilIndexList.Items.Add(format('Objects%d.pak', [i - 29]));
    end;
...
begin
  ...
  p := GetPiece(x, y);
  if p <> nil then begin
    with FrmMain.WilTiles[ShowIndex] do begin
      if p.bkimg >= 0 then
...
procedure TFrmObjEdit.DetailGridDrawCell(Sender: TObject; Col, Row: Longint; Rect: TRect; State: TGridDrawState);
var
  idx, max, wid: integer;
begin
  idx := Col;
