...
begin
  ...
  WilTiles := TWMImages.Create(nil);
  WilTiles.LibType := ltLoadBmp;
  WilTiles.MaxMemorySize := 1024000;
  WilTiles.FileName := 'Tiles.wil';
  WilTiles.Initialize;

  WilSmTiles := TWMImages.Create(nil);
  WilSmTiles.LibType := ltLoadBmp;
  WilSmTiles.MaxMemorySize := 1024000;
...
begin
  ...
  //MArr[xx, yy].FrImg := 0;
  lbl1.Caption := Format('B:%d  M:%d  F:%d  B:%d  M:%d  F:%d', [MArr[xx, yy].BkImg, MArr[xx, yy].MidImg, MArr[xx, yy].FrImg, NMArr[xx, yy].BkImg, NMArr[xx, yy].MidImg, NMArr[xx, yy].FrImg]);
  exit;
  if MainBrush = mbEraser then begin
          ...
          if (xx >= 0) and (xx < MAXX) and (yy >= 0) and (yy < MAXY) then begin
            ...
            if idx >= 0 then begin
              if MiddleTransparent1.Checked then
                ...
                WilSmTiles.DrawZoom(MapPaint.Canvas, xx, yy, idx, Zoom);
            end;
          end;
        end;
      end;
