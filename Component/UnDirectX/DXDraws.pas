...
//    function Fade2White(colorfrom: Integer): Longint;
  ...
  public
    ...
    procedure InitializeDisplayMode();
...
uses DXConsts, DXRender, D3DUtils;
  ...
  try
    ...
    try
      ...
      begin
        ...
        DDraw.IDraw.GetDisplayMode(DDSurfaceDesc);
        if DDSurfaceDesc.ddpfPixelFormat.dwRGBBitCount <> LongWord(Display.BitCount) then
          if not Display.DynSetSize(DDSurfaceDesc.dwWidth, DDSurfaceDesc.dwHeight, Display.BitCount) then
            raise EDXDrawError.CreateFmt(SDisplaymodeChange, [Display.Width, Display.Height, Display.BitCount]);
      end;
...
procedure TCustomDXDraw.InitializeDisplayMode;
var
  DDSurfaceDesc: TDDSurfaceDesc;
begin
...
var Changed: Boolean;
begin
  if (do3D in Options) and AsSigned(FD2D) then begin
...
{$IFDEF DrawHWAcc}
    with TPictureCollection(Self.GetPictureCollection) do
      if (do3D in FDXDraw.Options) and (FDXDraw.FD2D.FDDraw.FSurface = Dest) then begin
...
begin
  ...
  begin
    with TPictureCollection(Self.GetPictureCollection) do
      if (do3D in FDXDraw.FD2D.FDDraw.Options) and (FDXDraw.FD2D.FDDraw.FSurface = Dest) then begin
...
begin
  if {$IFNDEF DelphiX_Delphi3}Length(Texture){$ELSE}TexLen{$ENDIF} > maxTexBlock then begin
    D2DPruneAllTextures
  end;
end;
...
begin
  ...
  try
    ...
    //try
    with Texture[D2DMaxTextures - 1] do begin
      ...
      Width := VDIB.Width;
      Height := VDIB.Height;
      if VDIB.HasAlphaChannel then begin
        DIB2DXT(VDIB, T);
...
begin
  ...
  try
    ...
    if not dds.Modified then if T <> nil then begin
        if Assigned(T.VDIB) then
{$IFNDEF DelphiX_Delphi5}
        begin T.VDIB.Free; T.VDIB := nil; end;
{$ELSE}
          FreeAndNil(T.VDIB);
{$ENDIF}
...
begin
  ...
  if not FD2DTexture.{$IFDEF DelphiX_Spt4}CanFindTexture{$ELSE}CanFindTexture2{$ENDIF}(TexName) then begin
    ...
    if dds.Modified then begin {when modifying, load texture allways}
      if not FD2DTexture.{$IFDEF DelphiX_Spt4}LoadTextures{$ELSE}LoadTextures2{$ENDIF}(dds, Transparent, TexName) then
        Exit; {nepovede-li se to, pak ven}
    end;
...
begin
  ...
  begin
    ...
    if not (do3D in FDDraw.Options)
      ...
      or not (doHardware in FDDraw.Options)
      then
  ...
  if not FInitialized then
    if FCanUseD2D and Assigned(FDDraw.D3DDevice7) then begin
...
begin
  if FMirrorFlipSet = [rmfMirror] then begin
    {  X1,Y1             X2,Y1
    0  +-----------------+  1
       |                 |
       |                 |
  ...
  else if FMirrorFlipSet = [rmfFlip] then begin
    {  X1,Y1             X2,Y1
    0  +-----------------+  1
       |                 |
       |                 |
  ...
  else if FMirrorFlipSet = [rmfMirror, rmfFlip] then begin
    {  X1,Y1             X2,Y1
    0  +-----------------+  1
       |                 |
       |                 |
...
function TD2D.D2DMeshMapToWave(dds: TDirectDrawSurface; Transparent: Boolean;
  ...
  begin
    ...
    if not FD2DTexture.{$IFDEF DelphiX_Spt4}CanFindTexture{$ELSE}CanFindTexture2{$ENDIF}(TexName) then
      ...
      if not FD2DTexture.{$IFDEF DelphiX_Spt4}LoadTextures{$ELSE}LoadTextures3{$ENDIF}(dds, Transparent, TransparentColor, TexName) then Exit;
...
begin
  ...
  if (PatternIndex <> -1) {and (PatternRect <> ZeroRect)} then begin
    R := PatternRect;
...
{$ENDIF}
    ...
    {set of effect}
    with FDDraw.D3DDevice7 do begin
      case Effect of
...
begin
  ...
  if not FD2DTexture.{$IFDEF DelphiX_Spt4}CanFindTexture{$ELSE}CanFindTexture3{$ENDIF}(Color) then {when no texture in list try load it}
    if not FD2DTexture.{$IFDEF DelphiX_Spt4}LoadTextures{$ELSE}LoadTextures4{$ENDIF}(Color) then Exit; {on error occurr go out}
