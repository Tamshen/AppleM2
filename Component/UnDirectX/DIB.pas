...
type
  ...
  public
    ...
    function AssignAlphaChannel(ALPHA: TDIB; ForceResize: Boolean{$IFDEF DelphiX_Spt4} = False{$ENDIF}): Boolean;
    ...
    {Sephia effect}
    procedure SephiaEffect(Depth: Integer{$IFDEF DelphiX_Spt4} = 20{$ENDIF});
    ...
    procedure DrawDarken(SrcDIB: TDIB; X, Y, Width, Height,
      Frame: Integer);
    procedure DrawAdditive(SrcDIB: TDIB; X, Y, Width, Height: Integer; Alpha: Integer{$IFDEF DelphiX_Spt4} = 255{$ENDIF};
      Frame: Integer{$IFDEF DelphiX_Spt4} = 0{$ENDIF});
...
procedure MakeDib(out DIB: TDIB; const iWidth, iHeight, iBitCount: Integer; iFillColor: TColor{$IFDEF DelphiX_Spt4} = clBlack{$ENDIF}); {$IFDEF DelphiX_Spt4}overload; {$ENDIF}
procedure{$IFDEF DelphiX_Spt4}MakeDib{$ELSE}MakeDIB2{$ENDIF}(out DIB: TDIB; iBitmap: TBitmap); {$IFDEF DelphiX_Spt4}overload; {$ENDIF}
...
function TDIB.AssignAlphaChannel(ALPHA: TDIB; ForceResize: Boolean{$IFDEF DelphiX_Spt4} = False{$ENDIF}): Boolean;
...
begin
  if not AssignAlphaChannel(Value{$IFNDEF DelphiX_Spt4}, False{$ENDIF}) then
...
procedure TDIB.DoColorize(ForeColor, BackColor: TColor);
  procedure Colorize(src, dst: TDIB; iForeColor, iBackColor: TColor; iDither: Boolean{$IFDEF DelphiX_Spt4} = False{$ENDIF});
...
begin
  ...
  BB1.Assign(Self);
  BB2 := TDIB.Create;
  Colorize(BB1, BB2, ForeColor, BackColor{$IFNDEF DelphiX_Spt4}, False{$ENDIF});
  Self.Assign(BB2);
  BB1.Free;
  BB2.Free;
end;
...
const
  ...
  function Clamp(i, l, h: double): double;
  begin
    if i < l then
      result := l
    else if i > h then
  ...
  function IClamp(i, l, h: Integer): Integer;
  begin
    if i < l then
      result := l
    else if i > h then
  ...
  procedure rgb_to_hsl(r, g, b: Double; var h, s, l: Double);
  {$IFNDEF DelphiX_Spt4}
  ...
  procedure hsl_to_rgb(h, sl, l: Double; var r, g, b: Double);
  var
    v: Double;
    m, sv: Double;
    sextant: Integer;
...
procedure TDIB.SephiaEffect(Depth: Integer{$IFDEF DelphiX_Spt4} = 20{$ENDIF});
