...
function TServerWinSocket.GetConnectCount: Integer;
begin
  Result := FConnections.Count;
end;
...
begin
  ...
  if getsockopt(INVALID_SOCKET, SOL_SOCKET, SO_OPENTYPE, PChar(@OldOpenType), Len) = 0 then
  try
...
begin
  ...
  begin
    if FServerSocket.Connected then begin
      //Jason 2009-11-15 增加当ServerSocket停止监听时,将所有ClientSocket关闭
      for I := FServerSocket.ConnectCount - 1 downto 0 do begin
        CustomWinSocket := FServerSocket.Connections[I];
        if CustomWinSocket.Connected then
          CustomWinSocket.Close;
      end;
