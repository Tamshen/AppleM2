...
uses
  Windows, Classes, Graphics, Controls, StrUtils, SysUtils, Grids, Clipbrd, imm, HGE, HGECanvas, HGETextures,
  WIL, HUtil32, HGEBase, DirectXGraphics;
...
begin
  with surface do begin
    g_DXCanvas.Font.Color := bcolor;
    g_DXCanvas.TextOut(x - 1, y, str);
    g_DXCanvas.TextOut(x + 1, y, str);
    g_DXCanvas.TextOut(x, y - 1, str);
    g_DXCanvas.TextOut(x, y + 1, str);
    g_DXCanvas.Font.Color := fcolor;
    g_DXCanvas.TextOut(x, y, str);
  end;
end;
...
begin
  ...
  FWidth := 0;
  FItemIndex := -1;
  //g_DXCanvas.Font.Name := DEFFONTNAME;
  //g_DXCanvas.Font.Size := DEFFONTSIZE;
  FHeightOffset := 16;
  FAlpha := 220;
  FDlgWidth := 6;
  FDlgHeight := 6;
...
var
  ...
  d: TDXTexture;
begin
  g_DXCanvas.FillRect(left + 2, top + 2, FWidth - 4, FHeight - 4, $DC080808);
  for I := 0 to 3 do begin
    if FDlgInfo[I].WMImages = nil then Continue;
    d := FDlgInfo[I].WMImages.Images[FDlgInfo[I].Index];
    if d <> nil then begin
      g_DXCanvas.StretchDraw(Rect(FDlgInfo[I].Rect.Left + Left, FDlgInfo[I].Rect.Top + Top,
  ...
  for I := 4 to 7 do begin
    ...
    d := FDlgInfo[I].WMImages.Images[FDlgInfo[I].Index];
    if d <> nil then begin
      g_DXCanvas.Draw(FDlgInfo[I].Rect.Left + Left, FDlgInfo[I].Rect.Top + Top, d.ClientRect, d, True);
    end;
  end;
  g_DXCanvas.FillRect(FDlgWidth + Left, FDlgHeight + Top, FSelectWidth, FHeight - FDlgHeight * 2, $DC303030);
  with g_DXCanvas do begin
    nHeight := FDlgHeight;
...
begin
  ...
  Left := nLeft + 2;
  Top := nTop + 2;
  if (Left + FWidth + 20) >= GUIFScreenWidth then
    Left := _Max(0, Left - FWidth);
  if (Top + FHeight + 40) > GUIFScreenHeight then
...
begin
  ...
  for i := 0 to FItem.Count - 1 do begin
    if (g_DXCanvas.TextWidth(FItem[i]) + FDlgWidth * 2 + FSelectWidth + 24) > FWidth then
      FWidth := g_DXCanvas.TextWidth(FItem[i]) + FDlgWidth * 2 + FSelectWidth + 24;
...
begin
  ...
  if not FTransparent then begin
    g_DXCanvas.FillRect(dc.Left, dc.Top, Width, Height, $FF000000 or LongWord(Color));
    g_DXCanvas.RoundRect(dc.Left, dc.Top, dc.Right, dc.Bottom, FDFMoveColor);
  end;
...
begin
  if (FSurface <> nil) then begin
    ...
    g_DXCanvas.Draw(SurfaceX(Left), SurfaceY(Top), FSurface.ClientRect, FSurface, True);
  end;
end;
...
begin
  ...
  Result.boOpen := False;
  Result.nNameLen := g_DXCanvas.TextWidth(sName) + FImageWidth;
